...
function signSuperAdminToken(user) {
  return jwt.sign(
    ...
    { expiresIn: '12h', issuer: 'chatbotpro', audience: 'cbp:superadmin' }
  );
}
...
async function requireSuperAdmin(req, res, next) {
  ...
  try {
    const payload = jwt.verify(token, config.SUPERADMIN_JWT_SECRET, {
      issuer: 'chatbotpro',
      audience: 'cbp:superadmin',
    });
    if (payload.role !== 'superadmin') return res.status(401).json({ error: 'Sesión de superadmin inválida o expirada' });
